...
func TestProfileParity(t *testing.T) {
	...
	if got := profile.ResolveTypeID(api.PreparedMetadata{DiscType: "BDMV", UHD: "UHD", SourceSize: 60 * (1 << 30)}); got != "2" {
		t.Fatalf("type = %q", got)
	}
	...
	meta := api.PreparedMetadata{ReleaseName: "Example.2024.1080p.BluRay.REMUX.H.265.DD+ 5.1 Atmos-GRP", Audio: "DD+ 5.1", ExternalMetadata: api.ExternalMetadata{TMDB: &api.TMDBMetadata{Title: "Example", OriginalTitle: "Original Example"}}, SubtitleLanguages: []string{"Japanese"}}
	...
	keywordsMeta := api.PreparedMetadata{Region: "3", Distributor: "42", ExternalMetadata: api.ExternalMetadata{TMDB: &api.TMDBMetadata{Keywords: "one, two words, three,four,five,six,seven,eight,nine,ten,eleven,twelve"}}}
