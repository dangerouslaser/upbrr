...
func (h dupeSearcher) Search(ctx context.Context, meta api.PreparedMetadata, _ string) ([]api.DupeEntry, []string, error) {
	if h.http == nil {
		return nil, arSkip("AR handler misconfigured: no HTTP client"), nil
	}

	query := arSearchQuery(meta)
	if query == "" {
		return nil, arSkip("missing title for AR dupe search"), nil
	}
	...
	if err != nil || len(cookies) == 0 {
		...
		return nil, arSkip("missing valid AR cookies"), nil
	}
	...
	if err != nil {
		...
		return nil, arSkip("AR request failed"), nil
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, arSkip("AR search returned non-success status"), nil
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		...
		return nil, arSkip("AR response decode failed"), nil
	}
	if !strings.EqualFold(strings.TrimSpace(payload.Status), "success") {
		return nil, arSkip("AR API returned non-success status"), nil
	}
...
func arSkip(reason string) []string { return []string{"skip: " + strings.TrimSpace(reason)} }
