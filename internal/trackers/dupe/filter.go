...
func filterDupes(dupes []api.DupeEntry, meta api.PreparedMetadata, tracker string, registry *trackers.Registry) ([]api.DupeEntry, api.DupeMatch) {
	match := api.DupeMatch{}
...
func hasMatchingHDR(fileHDR, targetHDR map[string]struct{}, meta api.PreparedMetadata, tracker string, registry *trackers.Registry) bool {
	policy, _ := registry.LookupDupePolicy(tracker)
...
func isSameSeasonEpisodeResolutionMismatch(meta api.PreparedMetadata, name string, targetSeason string, targetEpisode string, targetResolution string) bool {
	if meta.TVPack || !strings.EqualFold(meta.ExternalIDs.Category, "TV") {
		return false
	}
