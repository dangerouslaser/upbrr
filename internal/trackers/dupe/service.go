...
// Service runs duplicate searches through capabilities registered for each tracker.
type Service struct {
	cfg      config.Config
	logger   api.Logger
	http     *http.Client
...
// NewServiceWithRegistry builds a duplicate-checking service and overlays
// tracker-owned search capabilities on the compatibility handler set.
// NewServiceWithRegistry returns a duplicate-check service backed by registry.
func NewServiceWithRegistry(cfg config.Config, logger api.Logger, registry *trackerspkg.Registry) *Service {
	if logger == nil {
		logger = api.NopLogger{}
	}
	httpClient := &http.Client{Timeout: 20 * time.Second}
	handlers := make(map[string]searchHandler)
	for _, name := range registry.Names() {
		if searcher, ok := registry.LookupDupeSearcher(name); ok {
			handlers[name] = searcher
			continue
		}
		if factory, ok := registry.LookupDupeSearcherFactory(name); ok {
			handlers[name] = factory.NewDupeSearcher(cfg, httpClient, logger)
		}
	}
	return &Service{
		cfg:      cfg,
		...
		handlers: handlers,
		filter: func(entries []api.DupeEntry, meta api.PreparedMetadata, tracker string, _ config.Config, _ api.Logger) ([]api.DupeEntry, api.DupeMatch) {
...
// Check searches the requested trackers and returns their normalized duplicate results.
