...
func (h dupeSearcher) Search(ctx context.Context, meta api.PreparedMetadata, _ string) ([]api.DupeEntry, []string, error) {
	if h.http == nil {
		return nil, ascSkip("ASC handler misconfigured: no HTTP client"), nil
	}
	if !meta.Anime && resolveASCIMDb(meta) == "" {
		return nil, ascSkip("missing IMDb ID for ASC dupe search"), nil
	}

	cookies, _, err := LoadCookies(ctx, h.cfg.MainSettings.DBPath)
	if err != nil || len(cookies) == 0 {
		return nil, ascSkip("missing valid ASC cookies"), nil
	}
	...
	if err != nil {
		return nil, ascSkip("ASC request failed"), nil
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, ascSkip("ASC search returned non-success status"), nil
	}

	doc, err := html.Parse(resp.Body)
	if err != nil {
		return nil, ascSkip("ASC response parse failed"), nil
	}
...
func ascSkip(reason string) []string { return []string{"skip: " + strings.TrimSpace(reason)} }
