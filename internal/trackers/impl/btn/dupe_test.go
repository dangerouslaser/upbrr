...
func btnTestSkipReason(notes []string) (string, bool) {
	if len(notes) != 1 {
		return "", false
	}
	note := strings.TrimSpace(notes[0])
	if !strings.HasPrefix(strings.ToLower(note), "skip: ") {
		return "", false
	}
	return strings.TrimSpace(note[len("skip: "):]), true
}
...
func TestBTNHandlerSkipsWithoutAPIKey(t *testing.T) {
	...
	handler := New().NewDupeSearcher(config.Config{}, http.DefaultClient, nil)
	...
	reason, ok := btnTestSkipReason(notes)
...
func TestBTNHandlerSkipsNonTV(t *testing.T) {
	...
	handler := New().NewDupeSearcher(configWithBTNAPIKey(), http.DefaultClient, nil)
	entries, notes, err := handler.Search(context.Background(), api.PreparedMetadata{
		SourcePath: "x",
		ExternalIDs: api.ExternalIDs{
			Category: "MOVIE",
...
func TestBTNHandlerUsesTrackerIDFirst(t *testing.T) {
	...
	handler := New().NewDupeSearcher(configWithBTNAPIKey(), payloads.client, nil)
