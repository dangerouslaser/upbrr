...
func (h dupeSearcher) Search(ctx context.Context, meta api.PreparedMetadata, _ string) ([]api.DupeEntry, []string, error) {
	...
	if apiKey == "" {
		return nil, []string{mtvSkip("missing api_key for tracker")}, nil
	}
	...
	default:
		...
		if query == "" {
			return nil, []string{mtvSkip("missing imdb/tmdb/tvdb id or title for MTV dupe search")}, nil
		}
		params.Set("q", query)
	}
	...
	if err != nil {
		return nil, []string{mtvSkip("MTV search failed")}, nil
	}
	...
	if err := xml.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, []string{mtvSkip("MTV response parse failed")}, nil
	}
...
func mtvSkip(reason string) string { return "skip: " + strings.TrimSpace(reason) }
