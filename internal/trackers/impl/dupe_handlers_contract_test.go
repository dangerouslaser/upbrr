...
type handlerContractSearcher interface {
...
func TestSiteHandlersSearch(t *testing.T) {
	...
	tests := []struct {
		...
		handler  func(cfg config.Config, client *http.Client) handlerContractSearcher
	...
	}{
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return btimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return flimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return ffimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return bjsimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return hdsimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return hdtimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return isimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
				if len(entries) != 1 || entries[0].Name != "Movie.2024.1080p.WEB-DL-GRP" {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return ptsimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
		...
		},
		{
			...
			handler: func(cfg config.Config, client *http.Client) handlerContractSearcher {
				return thrimpl.New().NewDupeSearcher(cfg, client, api.NopLogger{})
			},
			validate: func(t *testing.T, entries []api.DupeEntry) {
